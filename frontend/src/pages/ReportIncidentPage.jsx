...
const SEVERITIES = [
  { level: 'Low', color: 'bg-emerald-950/40 text-emerald-300 border-emerald-800/60', desc: 'Minor issue / non-life-threatening' },
  { level: 'Medium', color: 'bg-yellow-950/40 text-yellow-300 border-yellow-800/60', desc: 'Moderate hazard / requires attention' },
  { level: 'High', color: 'bg-amber-950/40 text-amber-300 border-amber-800/60', desc: 'Serious situation / urgent response' },
  { level: 'Critical', color: 'bg-red-950/50 text-red-300 border-red-800/70', desc: 'Life-threatening / immediate emergency' },
];
...
export default function ReportIncidentPage() {
  ...
  if (submittedIncident) {
    return (
      <main className="min-h-[calc(100vh-4rem)] w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-[var(--color-bg-surface)] border border-emerald-800/50 rounded-xl p-6 sm:p-8 shadow-2xl space-y-6 text-center max-w-3xl mx-auto animate-fadeIn">
          <div className="w-16 h-16 rounded-full bg-emerald-950/60 border border-emerald-600/50 text-emerald-400 flex items-center justify-center mx-auto text-2xl">
            ✓
          </div>
          ...
          <div className="bg-[var(--color-bg-elevated)] border border-[var(--color-bg-border)] rounded-xl p-5 text-left space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-[var(--color-bg-border)]">
              ...
              <span className="font-mono text-base font-bold text-[var(--color-primary)] px-3 py-1 rounded bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/30">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div>
                ...
                <span className="font-mono text-xs uppercase font-semibold text-emerald-400 bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-800/40 inline-block mt-0.5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <button
              ...
              className="w-full sm:w-auto px-6 py-3 rounded-lg text-sm font-semibold bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-hover)] transition-all cursor-pointer shadow-sm"
            >
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col gap-4">
      {/* Header */}
      <div className="border-b border-[var(--color-bg-border)] pb-3">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-[var(--color-text-primary)] flex items-center gap-2.5">
          <svg className="w-6 h-6 sm:w-7 sm:h-7 text-[var(--color-primary)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
      ...
      {serverError && (
        <div className="p-3 rounded-xl bg-red-950/40 border border-red-800/60 text-red-200 text-xs flex items-start gap-3">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
          ...
          <div className="bg-[var(--color-bg-surface)] border border-[var(--color-bg-border)] rounded-xl p-5 shadow-lg space-y-4 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full px-3.5 py-2 rounded-lg text-xs sm:text-sm bg-[var(--color-bg-elevated)] border text-[var(--color-text-primary)] focus:outline-none transition-all ${
                    fieldErrors.title ? 'border-red-700 focus:border-red-500' : 'border-[var(--color-bg-border)] focus:border-[var(--color-primary)]'
                  }`}
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    disabled={isSubmitting}
                    className="w-full px-3.5 py-2 rounded-lg text-xs sm:text-sm bg-[var(--color-bg-elevated)] border border-[var(--color-bg-border)] text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] transition-all cursor-pointer"
                  >
                    {CATEGORIES.map((cat) => (
                      ...
                      return (
                        <button
                          ...
                          className={`py-1.5 px-2 rounded-lg text-xs font-semibold border transition-all cursor-pointer text-center ${
                            isSelected
                              ? `${s.color} ring-2 ring-offset-1 ring-offset-[var(--color-bg-surface)] ring-[var(--color-primary)]`
                              : 'bg-[var(--color-bg-elevated)] text-[var(--color-text-secondary)] border-[var(--color-bg-border)] hover:border-[var(--color-primary)]/50'
                          }`}
                        >
                          {s.level}
                        </button>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  disabled={isSubmitting}
                  className={`w-full px-3.5 py-2 rounded-lg text-xs sm:text-sm bg-[var(--color-bg-elevated)] border text-[var(--color-text-primary)] focus:outline-none transition-all ${
                    fieldErrors.description ? 'border-red-700 focus:border-red-500' : 'border-[var(--color-bg-border)] focus:border-[var(--color-primary)]'
                  }`}
                />
          ...
          <div className="bg-[var(--color-bg-surface)] border border-[var(--color-bg-border)] rounded-xl p-5 shadow-lg flex flex-col justify-between space-y-3">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-[var(--color-text-primary)] flex items-center gap-2 border-b border-[var(--color-bg-border)] pb-2">
              <span className="w-2 h-2 rounded-full bg-[var(--color-primary)]"></span>
        ...
        <div className="bg-[var(--color-bg-surface)] border border-[var(--color-bg-border)] rounded-xl p-4 sm:p-5 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            type="button"
            onClick={handleResetForm}
            className="w-full sm:w-auto px-6 py-3 rounded-lg text-sm font-semibold bg-[var(--color-bg-elevated)] text-[var(--color-text-primary)] border border-[var(--color-bg-border)] hover:border-[var(--color-primary)] transition-all text-center cursor-pointer"
          >
            Cancel / Clear Form
          </button>
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="w-full sm:w-auto px-8 py-3.5 rounded-lg text-sm font-bold bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-hover)] disabled:opacity-50 transition-all cursor-pointer shadow-lg flex items-center justify-center gap-2.5"
          >
            {isSubmitting ? (
              <>
